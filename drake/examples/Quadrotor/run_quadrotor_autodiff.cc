...
#include "drake/multibody/rigid_body_tree_auto_diff.h"
...
int do_main() {
  ...
  auto tree2 = std::make_unique<RigidBodyTreeAutoDiff<double>>();
  //int a = 2;
  tree2->get_num_actuators();
  tree2->RigidBodyTreeAutoDiff::PrintValue();

  /*

  auto base = std::make_unique<RigidBody<AutoDiffXd>>();
  base->set_name("base");
  base->set_mass(0.5);

  RigidBody<AutoDiffXd>* base_ptr = tree2->add_rigid_body(std::move(base));

  base_ptr->add_joint(&tree2->world(), std::make_unique<QuaternionFloatingJoint>(
      "base", Eigen::Isometry3d::Identity()));

  tree2->compile();
  */
  //auto plant =
  //    builder.template AddSystem<systems::RigidBodyPlant<AutoDiffXd>>(std::move(tree));
