...
bool CompareToClone(const RigidBodyTree<double>& tree,
  ...
  for (int i = 0; i < static_cast<int>(tree.loops.size()); ++i) {
    ...
    const RigidBodyLoop<T>* other_loop = &clone.loops.at(i);
...
template bool CompareToClone<double>(const RigidBodyTree<double>& tree,
                                     const RigidBodyTree<double>& clone);
template bool CompareToClone<drake::AutoDiffXd>(
    const RigidBodyTree<double>& tree,
    const RigidBodyTree<drake::AutoDiffXd>& clone);
